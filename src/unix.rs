...
lazy_static! {
    static ref DEVICE: Result<File, std::io::Error> = OpenOptions::new().append(true).open(FILE);
}
...
pub fn beep_indefinite(hertz: u32) -> Result<()> {
    let period_in_clock_cycles = TIMER_FREQUENCY.checked_div(hertz).unwrap_or(0);
